<template>
  <header class="header container header__container">
    ...
    <div class="header__link-group">
      <a
        @click="$router.push('/')"
        class="header__link header__link-disabled header__login"
      >
        Войти
        <svg
          width="18"
      ...
      <a @click="$router.push('/reg')" class="header__link header__reg">
        Зарегистрироваться
        <svg
          class="header__svg"
          width="24"
  ...
  <main class="main">
    <div class="container main__container full-screen">
      <form @submit.prevent="handleLogin" class="main__form form">
  ...
  setup() {
    ...
    const handleLogin = async () => {
      if (isFormValid.value) {
        const result = await userApi.signin(email.value, password.value);
        if (!result.error) {
          router.push("/home");
        } else {
          showError();
